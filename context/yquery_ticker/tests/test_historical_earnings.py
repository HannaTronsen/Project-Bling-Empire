...
class test_historical_earnings(unittest.TestCase):
    ...
    def test_is_consistently_up_trending(self):
        result, _ = TimeSeriesDataCollection.is_consistently_up_trending(chart_list=self.quarterly_earnings_data_up_trending_list, attribute='actual'); assert result is True
        ...
        result, _ = TimeSeriesDataCollection.is_consistently_up_trending(chart_list=self.quarterly_earnings_data_chart_expected_list, attribute='actual'); assert result is False
        result, _ = TimeSeriesDataCollection.is_consistently_up_trending(chart_list=self.quarterly_financials_data_chart_expected_list, attribute='revenue'); assert result is False
        result, _ = TimeSeriesDataCollection.is_consistently_up_trending(chart_list=self.quarterly_earnings_data_chart_expected_list, attribute='estimate'); assert result is False
        ...
        result, _ = TimeSeriesDataCollection.is_consistently_up_trending(chart_list=self.negative_values_list, attribute='revenue'); assert result is False
        result, _ = TimeSeriesDataCollection.is_consistently_up_trending(chart_list=self.negative_values_list, attribute='earnings'); assert result is True
        ...
        result, _ = TimeSeriesDataCollection.is_consistently_up_trending(chart_list = [1,2,3]); assert result is True
        result, _ = TimeSeriesDataCollection.is_consistently_up_trending(chart_list = [0,-1,-2,-3]); assert result is False
        result, _ = TimeSeriesDataCollection.is_consistently_up_trending(chart_list = [-3,-2,-1,0]); assert result is True
        result, _ = TimeSeriesDataCollection.is_consistently_up_trending(chart_list = [1,3,2]); assert result is False
        result, _ = TimeSeriesDataCollection.is_consistently_up_trending(chart_list = [1,3]); assert result is True
    ...
    def test_get_consecutive_upward_trend_interval(self):
        result, interval = TimeSeriesDataCollection.is_consistently_up_trending(chart_list = [0,1,4,3,4]); assert result is False and interval is 2
        result, interval = TimeSeriesDataCollection.is_consistently_up_trending(chart_list=self.quarterly_earnings_data_dip_in_up_trend_list, attribute='actual'); assert result is False and interval is 3
        result, interval = TimeSeriesDataCollection.is_consistently_up_trending(chart_list=self.quarterly_earnings_data_dip_in_up_trend_list, attribute='estimate'); assert result is False and interval is 2
        result, interval = TimeSeriesDataCollection.is_consistently_up_trending(chart_list=self.negative_values_list, attribute='revenue'); assert result is False and interval is 1
        result, interval = TimeSeriesDataCollection.is_consistently_up_trending(chart_list = [1,3,2]); assert result is False and interval is 1
        result, interval = TimeSeriesDataCollection.is_consistently_up_trending(chart_list = [0,-1,-4,-3,-4]); assert result is False and interval is 1
        result, interval = TimeSeriesDataCollection.is_consistently_up_trending(chart_list = [0,-1,-4,-3,-5,-4]); assert result is False and interval is 2

    def test_passes_percentage_increase_requirements(self):
        series = [100, 50]
        assert TimeSeriesDataCollection._passes_percentage_increase_requirements(series=series, percentage_requirement=50) is True
        assert TimeSeriesDataCollection._passes_percentage_increase_requirements(series=series, percentage_requirement=51) is False
        assert TimeSeriesDataCollection._passes_percentage_increase_requirements(series=series, percentage_requirement=49) is True
        assert TimeSeriesDataCollection._passes_percentage_increase_requirements(series=series, percentage_requirement=101) is False
        assert TimeSeriesDataCollection._passes_percentage_increase_requirements(series=series, percentage_requirement=100) is False
        assert TimeSeriesDataCollection._passes_percentage_increase_requirements(series=[33.33, 50, 100, 0], percentage_requirement=1) is False
        assert TimeSeriesDataCollection._passes_percentage_increase_requirements(series=[26.67, 281.82], percentage_requirement=26) is True
